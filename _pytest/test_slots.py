...
from conversationinsights.slots import *


class SlotTestCollection(object):
    """Tests every slot needs to fulfill. Each slot can declare further tests on its own."""
    ...
    def invalid_value(self, request):
        """Values, that should be handled gracefully but where the featurization is not defined."""
        raise NotImplementedError
    ...
    def test_featurization(self, value_feature_pair):
        ...
        assert len(slot.as_feature()) == slot.feature_dimensionality(), "Wrong feature dimensionality"

        # now reset the slot to get initial value again
        slot.reset()
        assert slot.value == slot.initial_value, "Slot should be reset to its initial value"

    def test_empty_slot_featurization(self):
        slot = self.create_slot()
        assert slot.value == slot.initial_value, "An empty slot should be set to the initial value"
...
class TestCategoricalSlot(SlotTestCollection):
    def create_slot(self):
        return CategoricalSlot("test", values=[1, "two", {"three": 3}, None])
