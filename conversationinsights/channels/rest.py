...
class HttpInputChannel(InputChannel):
    ...
    to be passed in. These components define API endpoints, e.g. there can be a 3rd party REST endpoint.
    This channel will then start a HTTP server for accepting the incoming HTTP requests and
    redirecting them to the appropriate listener components."""
    ...
    def _record_messages(self, on_message):
        ...
        for component in self.listener_components:
            if not self.url_prefix or self.url_prefix == "" or self.url_prefix == "/":
